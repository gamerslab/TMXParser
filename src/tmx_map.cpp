...
namespace TMX {
    Map* parse(const char* dirname, const char* filename) {
        Map* map = new Map();

        std::stringstream path;
        path << dirname << filename << ".tmx";
        ...
        map->background_color = get_attribute(root_node, "backgroundcolor", "000000");
        ...
        for(rapidxml::xml_node<> *tileset_node = root_node->first_node("tileset"); tileset_node;
            ...
            TSX::Tileset* tileset = TSX::parse(dirname, tileset_node->first_attribute("source")->value());
        ...
        for(rapidxml::xml_node<> *layer_node = root_node->first_node("layer"); layer_node;
            ...
            map->tile_layers[layer.name] = layer;
        }
        ...
        for(rapidxml::xml_node<> *oGroup_node = root_node->first_node("objectgroup"); oGroup_node;
            ...
            oGroup.color = get_attribute(oGroup_node, "color", "000000");
            oGroup.opacity = std::atof(get_attribute(oGroup_node, "opacity", "1.0"));
            oGroup.visible = (bool) std::atoi(get_attribute(oGroup_node, "visible", "1"));
            ...
            for(rapidxml::xml_node<> *object_node = oGroup_node->first_node("object"); object_node;
                ...
                object.gid = (unsigned int) atoi(get_attribute(object_node, "gid", "0"));
        ...
        for(rapidxml::xml_node<> *image_node = root_node->first_node("imagelayer"); image_node; image_node = image_node->next_sibling("imagelayer")) {
            ...
            imgLayer.opacity = std::atof(get_attribute(image_node, "opacity", "1"));
    ...
    void Map::Print() {
        ...
        for(const auto &name_layer : tile_layers) {
            const TileLayer &layer = name_layer.second;
